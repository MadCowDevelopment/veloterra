-- Classification v2 changes prices only, not which OSM features are candidates,
-- so existing discovery coverage remains valid.
update public.landmark_discovery_areas
set classification_version = 2
where classification_version < 2;

-- Release quota consumed while v2 coverage was incorrectly treated as stale.
delete from public.landmark_discovery_requests as requests
using public.landmark_discovery_areas as areas
where requests.area_key = areas.area_key
  and areas.classification_version >= 2;

drop function if exists public.claim_landmark_discovery(text);

create function public.claim_landmark_discovery(
  p_area_key text,
